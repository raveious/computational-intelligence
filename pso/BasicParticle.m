classdef BasicParticle < handle
    %BASICPARTICLE BasicParticle behavior for PSO
    ...
    methods
        function obj = BasicParticle(fitnessFunc, initialPosition)
            %BASICPARTICLE Construct an instance of this class
        ...
        function [newPos, newFitness] = move(obj, gBestPos)
            % Update velocity
            inertia = (0.6) .* obj.velocity;
            pvBest = (0.7) * rand() .* (obj.personalBestLocation - obj.position);
            gvBest = (0.85) * rand() .* (gBestPos - obj.position);
